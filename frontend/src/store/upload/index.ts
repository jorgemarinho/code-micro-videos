...
export const { Types, Creators } = createActions<{
    ...
    updateProgress(payload: Typings.UploadProgressAction['payload']): Typings.UploadProgressAction
...
function addUpload(state = INITIAL_STATE, action: Typings.AddUploadAction): Typings.UploadState {
    ...
    const index = findIdexUpload(state, action.payload.video.id);
...
function updateProgress(state: Typings.UploadState = INITIAL_STATE, action: Typings.UploadProgressAction): Typings.UploadState {
...
function setUploadError(state: Typings.UploadState = INITIAL_STATE, action: Typings.SetUploadErrorAction) {
...
function findIndexUploadAndFile(state: Typings.UploadState, videoId, fileField): {indexUpload?, indexFile?} {
    ...
    const indexUpload  = findIdexUpload(state, videoId);
    ...
    const upload = state.uploads[indexUpload];
    const indexFile = findIdexIndex(upload.files, fileField);
...
function findIdexUpload(state: Typings.UploadState, id: string) {
    return state.uploads.findIndex((upload) => upload.video.id === id);
}

function findIdexIndex(files: Array<{fileField}>, fileField: string) {
